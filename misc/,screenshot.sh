...
_IMAGE_OUTPUT="/home/abhijit/inbox/screenshots/${_THIS_DATE}.png"
_VIDEO_OUTPUT="/home/abhijit/inbox/videos/${_THIS_DATE}.mp4"

function image-selection () {
    echo "Screenshot has been saved."
...
function video-selection-start () {
    ...
    # Print message
    echo "Video recording has started."
}

function video-stop() {
   pkill --signal INT --pidfile /tmp/ffmpeg-record.pid && firefox "$(cat /tmp/ffmpeg-record.filename)"
   echo "Video recording has stopped."
   xdg-open "$_VIDEO_OUTPUT"
}
