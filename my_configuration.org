...
* Development Environment
  Always enable lsp mode before coding, using ~lsp~
** Additional Packages
...
* Copy Paste
** kill-ring
~variable~ Its a single list of text-blocks, Its shared by all buffers so we can kill in one buffer and yank in another buffer.
There are also ways to use ~register~ just like vim. 
** Registers
As we are using evil, using registers is the sensible way.
~M-x view-register RET r~
"ayy -> yank in register a
"sdd -> cut in register s

Use ~:reg~ to see all currently-defined registers
"+p -> To paste from system clipboard on Linux
** Default cut/delete, yank registers
[[https://stackoverflow.com/questions/1497958/how-do-i-use-vim-registers][Amazing explanation.]]
0 register -> yank text(default)
" register -> its the default register from where ~p~ command takes data from. yanking, deleting, cutting etc changes this default register.
*** Copy, then delete, then paste
yy (saved in 0 and ")
dd (saved in ")
"0p (paste from 0 register)

* Replace
[[https://www.oreilly.com/library/view/learning-gnu-emacs/1565921526/ch04s02.html][More detail, includes recursive-query.]]
Incremental replace is my goal. Using ~:%s/text1/newtext~ just replaces the first occurence in a line which is annoying. I wish for a more robust replace procedure.
** Emacs Query
   Use ~query-replace~ and ~replace-string~
- In query-replace:
  - press q to quit
  - y, n 
...
*** Select line-range and replace
:6,10s/foo/bar/g [replace all foo in line 6 to 10]
*** Ask before replacing
    :%s/text1/newText/gc
*** Replace in whole file
:%s/foo/bar/g [replace in whole file]
*** Replace in currently, VERY HANDY
:s/<search_string>/<replace_string> [Current line]
*** Replace in different ranges of line using &&
If you want to replace in line [6-10], [14-18] and [20-23] and not in-between-
:6,10s/<search_string>/<replace_string>/g | 14,18&& | 20,23&&
*** Replace using for loop
Hell, you can even use a loop.
:for range in split('6,10 14,18')| exe range 's/<search_string>/<replace_string>/g' | endfor
-- what is exe? Run ~C-h k :~ to understand.
*** Select lines/region where you want to replace in ~VISUAL~ mode
Or even better, you can select the lines in visual-mode and do
...
* Create scratch buffer
  Sometimes we mistakenly kill the scratch buffer. So running this function will bring back scratch mode.
  [[https://stackoverflow.com/questions/234963/re-open-scratch-buffer-in-emacs][source.]]
#+begin_src emacs-lisp
...
#+RESULTS:
: create-scratch-buffer
* WILL_WORK_ON_IT Split 3rd and fifth windows
Split these windows.
fifth: vterm + config.org
second: vterm+ scratch
* org-timer
Its so amazing for productivity. ~pomodoro clock~
org-timer-set-timer
org-timer-continue-or-pause
org-timer-stop
[[https://www.youtube.com/watch?v=JbHE819kVGQ][Source Video.]]
#+begin_src emacs-lisp
(setq org-clock-sound "~/.emacs.d/misc/ding.wav")
#+end_src
* INCOMPLETE dont use backspace!
#+begin_src emacs-lisp
...
* WILL_WORK_ON_IT org-timer - save time !
* auto-save.el
  This is to aid me in remote file editing. While editing remote files, the occassional auto-saves slows down the processes for brief moments which can be annoying. So we set ~/manual-installation/autosve to save all autosave-files. As its a local folder, i don't experience any slow down while editing remote files.
  [[https://github.com/emacsmirror/ohio-archive/blob/master/old-archive/misc/auto-save.el][Documetation.]]
#+begin_src emacs-lisp
...
#+RESULTS:
: /home/abhijit/manual-installation/autosave/
* WILL_WORK_ON_IT Emacs for Web Development
  First, install it using package-install.
#+begin_src emacs-lisp
...
: t
* lorem ipsum
[[https://github.com/jschaf/emacs-lorem-ipsum][Github Documentation]]
First, try package-install lorem-ipsum
...
#+RESULTS:
: abj/split-and-open
* Set Inline Image Height-Width In Org-Mode
~#+ATTR_ORG: :width 500~
We have to set emacs to not follow the actual width of the image. Then we can set the image width using ~#+ATTR_ORG: :width 500~.
[[https://stackoverflow.com/questions/11670654/how-to-resize-images-in-org-mode][Source]]
#+begin_src emacs-lisp
(setq org-image-actual-width nil)
#+end_src
* Org Presentation
~htmlize~ package is needed to highlight syntax in code. org-export using reveal.js does not highlight code by default. We need htmlize for that.
#+begin_src emacs-lisp
...
* Data Science
package-install ein
[[https://github.com/millejoh/emacs-ipython-notebook#ob-ein][Deatail here on github]]
#+begin_src emacs-lisp
(use-package ein
  :init 
  (setq ein:polymode t
...
#+end_src
* ob-shell.el
Its necessary for writing bash scripts inside org file.
#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/")
(load "ob-shell.el")
#+end_src
...
#+begin_src emacs-lisp
 (defun abj/open-VM()
   ...
   ;;(eshell)
   (multi-vterm)
   (rename-buffer "Remote Virtual Machine")
   ;;(eshell-exec-visual "ls")
 )
...
* Remote file editing using SSH
  Simply do ~/ssh:root@192.168.1.177: /home/test-user/~ to go there. [[https://stackoverflow.com/questions/20624024/what-is-the-best-way-to-open-remote-files-with-emacs-and-ssh][Details]]
- But vterm opens a ugly terminal in ssh. What to do? Use eshell! eshell works fine with ssh/tramp. [[https://stackoverflow.com/questions/1346688/ssh-through-emacs-shell][Details]] 
#+begin_src emacs-lisp
;(use-package ssh) ;; Not really the best one.
#+end_src
* WILL_WORK_ON_IT Custom eshell theme
[[https://www.youtube.com/watch?v=kCCIudu53Zg][Video by System Crafters]]
  counsel-color-emacs to get emacs colors.
  describe-face to get the face name.
...
* Diagram 
** plantuml
[[https://plantuml.com/emacs][Installation guide and test usage]]
Difficult syntax
#+begin_src emacs-lisp
(setq org-plantuml-jar-path (expand-file-name "/home/abhijit/.emacs.d/plantuml-1.2021.14.jar"))
...
* evil org-link
#+begin_src emacs-lisp
(defun abj/evil-org-mode-enable ()
(interactive)
(setq evil-want-minibuffer t)
)

(defun abj/evil-org-mode-disable ()
(interactive)
...
#+RESULTS:
: abj/evil-org-mode-disable
* Mathematics format in org mode 
#+begin_src emacs-lisp
(use-package ob-sagemath)
#+end_src
* C/C++ for org mode
  Now we can execute a c++ code block inside org mode.
#+begin_src emacs-lisp
(org-babel-do-load-languages
...
#+RESULTS:
* Put bracket around the selected region
[[https://stackoverflow.com/questions/2951797/wrapping-selecting-text-in-enclosing-characters-in-emacs][Source.]]
By default, ~M-(~ works fine.
#+begin_src emacs-lisp
